...
"""
    ...
    def do_publish(self, arg):
        """Auto-detect and publish packages to registries.

        Usage: publish [path] [--registry NAME] [--all-registries] [--dry-run]

        Examples:
            publish                          # Publish current repo
            publish /repos/myproject         # Publish specific repo
            publish --registry pypi          # Force specific registry
            publish --all-registries         # Publish to all configured registries
            publish --dry-run                # Preview without publishing
            publish /by-language/Python      # Publish all Python repos
        ...
        while i < len(args):
            if args[i] == '--registry' and i + 1 < len(args):
                registry = args[i + 1]
                i += 2
            elif args[i] == '--all-registries':
                all_registries = True
                i += 1
            elif args[i] == '--dry-run':
                dry_run = True
                i += 1
            elif not args[i].startswith('-') and vfs_path is None:
                vfs_path = args[i]
                i += 1
            else:
                i += 1

        # Default to current directory if in real FS mode, otherwise use cwd
        if vfs_path is None:
            if self.in_real_fs and self.real_fs_repo:
                vfs_path = self.real_fs_repo
            else:
                # Use current VFS directory
                vfs_path = str(self.cwd)

        # Import and call publish handler
        from ..commands.publish import publish_handler
        import click

        try:
            # Create minimal context and invoke
            publish_handler.callback(
                vfs_path=vfs_path,
                registry=registry,
                all_registries=all_registries,
                dry_run=dry_run,
                json_output=False  # Shell uses pretty output
            )
        except click.Abort:
            pass
        except Exception as e:
    ...
    def do_export(self, arg):
        ...
        Examples:
            export markdown              # Generate markdown portfolio
            export hugo --output ./site  # Generate Hugo site
            export csv --output repos.csv  # Export to CSV
        """
        if not arg:
            print("Usage: export <format> [options]")
            print("Formats: markdown, hugo, html, csv, json")
            return
        ...
        format_type = args[0] if args else None
        if not format_type:
        ...
        if '--output' in args:
            ...
            if out_idx + 1 < len(args):
                output = args[out_idx + 1]

        try:
            # Call export handler
            # Note: This is simplified - full implementation would parse all options
            print(f"[yellow]Exporting as {format_type}...[/yellow]")
            print(f"[dim]Use CLI for full export options: ghops export {arg}[/dim]")

            # For now, suggest using CLI
            print(f"\n[cyan]Tip:[/cyan] For full export features, use:")
            print(f"  ghops export {arg}")
